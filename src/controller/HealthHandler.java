    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            if (conn.isValid(2)) {
                ...
                HttpResponseUtil.sendResponse(exchange, 200, "{\"status\":\"UP\",\"database\":\"UP\"}");
            } else {
                logger.warn("Readiness check failed: database connection is not valid");
                HttpResponseUtil.sendResponse(exchange, 503, "{\"status\":\"DOWN\",\"database\":\"DOWN\"}");
            }
        } catch (SQLException e) {
            logger.error("Readiness check failed: database unreachable", e);
            HttpResponseUtil.sendResponse(exchange, 503,
